  ...
  if (variant === "faixa") {
    return (
      <div className="mx-auto max-w-[var(--container-max,1240px)] px-4 pt-[42px] sm:px-6">
        <div className="flex gap-[18px] overflow-x-auto rounded-xl border border-[var(--store-line)] bg-[var(--store-surface)] px-6 py-5 [scrollbar-width:thin]">
          {REVIEWS.map((r) => (
            <div key={r.name} className="flex min-w-[300px] flex-1 items-start gap-3.5">
              <Stars n={r.stars} className="mt-0.5 flex text-[13px] text-[#B45309]" />
              <div>
  ...
  return (
    <div className="mx-auto max-w-[var(--container-max,1240px)] px-4 pt-[42px] sm:px-6">
      <div className="mb-[18px] flex items-end justify-between">
        <h2 className="font-display text-[26px] font-extrabold">{title}</h2>
        {media && <div className="flex items-center gap-2 text-sm font-semibold text-[var(--store-muted)]"><Star weight="fill" className="text-[17px] text-[#B45309]" /><b className="text-[var(--store-ink)]">{media}</b></div>}
      </div>
      <div className="grid gap-[18px] md:grid-cols-3">
        {REVIEWS.map((r) => (
          <div key={r.name} className="flex flex-col gap-3.5 store-card rounded-xl p-6">
            <Stars n={r.stars} className="flex text-[15px] text-[#B45309]" />
