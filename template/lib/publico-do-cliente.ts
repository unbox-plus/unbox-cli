...
import { UnboxCustomerClient } from "@/lib/unbox/customer";
import { getStoreClient } from "@/lib/unbox/store";
...
async function dadosDaConta(token: string, doc: ContentDocument | null): Promise<DadosDoCliente> {
  ...
  const c = new UnboxCustomerClient({ customerToken: token, loja: await getStoreClient() });
  const [produtos, assinaturas, conta] = await Promise.all([
    pede("comprou") ? c.produtosComprados().catch((e) => (registrar("pedidos", e), [] as string[])) : ([] as string[]),
